...
extension UIBarButtonItem {
    ...
    static func Jh_backItem(imageName:String,target:AnyObject,action:Selector) -> UIBarButtonItem {
        return Jh_systemItem(title: "", titleColor: UIColor.white, imageName: imageName, target: target, action: action,contentHorizontalAlignment: .left,isBack: true)
    }
    ...
    /// 快速初始化一个UIBarButtonItem，内部是按钮 imageName 接收SF Symbols 5
    static func Jh_systemItem(title:String,titleColor:UIColor,imageName:String,target:AnyObject,action:Selector,
                              contentHorizontalAlignment:UIControl.ContentHorizontalAlignment,isBack:Bool=false) -> UIBarButtonItem {
        let button = UIButton()
        if (title.count>0) {
            button.setTitle(title, for: .normal)
